...
def connectall(usersettings=None):
        ...
        for settings_path in settings_paths:
            try:
                if os.path.exists(settings_path):
                    ...
                    root = tree.getroot()
                    input_port = (
                        root.find("./input_port").text
                        if root.find("./input_port") is not None
                        else "default"
                    )
                    secondary_input_port = (
                        root.find("./secondary_input_port").text
                        if root.find("./secondary_input_port") is not None
                        else "default"
                    )
                    settings_found = True
    ...
    if secondary_input_port_id not in port_list:
        print(
            f"Secondary input port ID '{secondary_input_port_id}' not found in available ports"
        )
        secondary_input_port_id = None

    # Connect the ports if both are found
    if input_port_id and secondary_input_port_id:
        print("Disconnecting all existing MIDI connections...")
        # First disconnect all existing connections
        subprocess.call("aconnect -x", shell=True)

        print(
            f"Connecting {input_port} ({input_port_id}) to {secondary_input_port} ({secondary_input_port_id})"
        )
        # Two-way connection: input -> secondary and secondary -> input
        result1 = subprocess.call(
            f"aconnect {input_port_id} {secondary_input_port_id}", shell=True
        )
        result2 = subprocess.call(
            f"aconnect {secondary_input_port_id} {input_port_id}", shell=True
        )
        ...
        print(
            f"Could not find ports: input_port='{input_port}', secondary_input_port='{secondary_input_port}'"
        )
